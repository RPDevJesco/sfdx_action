...
function SwimLaneLayout() {
    window.go.LayeredDigraphLayout.call(this);
    this.alignOption = window.go.LayeredDigraphLayout.AlignAll;
    ...
    this.lanePositions = new window.go.Map();  // lane names --> start columns, left to right
    this.laneBreadths = new window.go.Map();  // lane names --> needed width in columns

    // internal state
    this._layers = null;
    this._neededSpaces = null;
}
window.go.Diagram.inherit(SwimLaneLayout, window.go.LayeredDigraphLayout);
...
SwimLaneLayout.prototype.doLayout = function(coll) {
    ...
    this._layers = null;
    this._neededSpaces = null;
    window.go.LayeredDigraphLayout.prototype.doLayout.call(this, coll);
...
SwimLaneLayout.prototype.setupLanes = function() {
    ...
    var layout = this;
    var laneNameSet = new window.go.Set().addAll(this.laneNames);
    var laneIndexes = new window.go.Map();  // lane names --> index when sorted
    var layers = [];
    this._layers = layers;
...
SwimLaneLayout.prototype.computeLanes = function() {
    ...
    var lwidths = new window.go.Map();  // reused for each layer
    for (var i = 0; i <= this.maxLayer; i++) {
...
window.SwimLaneLayout = SwimLaneLayout;