class PoolLayout extends window.go.GridLayout {
    constructor() {
        super();
        ...
        this.cellSize = new window.go.Size(1, 1);
        this.wrappingColumn = Infinity;
        this.wrappingWidth = Infinity;
        this.spacing = new window.go.Size(0, 0);
        this.alignment = window.go.GridLayout.Position;
    }
    ...
    doLayout(coll) {
        ...
        diagram.findTopLevelGroups().each(lane => {
            if (!(lane instanceof window.go.Group)) return;
    ...
    computeLaneSize(lane) {
        ...
        const sz = new window.go.Size(lane.isSubGraphExpanded ? this.MINBREADTH : 1, this.MINLENGTH);
